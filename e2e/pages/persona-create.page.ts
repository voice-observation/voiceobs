...
export class PersonaCreatePage {
  readonly page: Page;
  ...
  constructor(page: Page) {
    ...
    this.submitButton = page.locator('button:has-text("Create Persona")');
    this.cancelButton = page.locator('a[href="/personas"]:has-text("Cancel")');
    this.validationErrors = page.locator('.text-destructive');
  }

  async goto() {
    await this.page.goto('/personas/new');
    await this.page.waitForLoadState('networkidle');
  }

  async isLoaded(): Promise<boolean> {
    try {
      await this.page.waitForURL(/\/personas\/new/, { timeout: 10000 });
      return true;
    } catch {
      return false;
    }
  ...
  async fillForm(data: {
    ...
    if (data.traits && data.traits.length > 0) {
      await this.selectTraits(data.traits);
    }
    if (data.voiceProvider) {
      await this.selectVoiceProvider(data.voiceProvider);
    }
    if (data.voiceModel) {
      await this.selectVoiceModel(data.voiceModel);
    }
  }
