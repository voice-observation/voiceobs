...
def _generate_recommendations(data: ReportData) -> list[str]:
    ...
    slow_failures = [
        f for f in data.failures.failures if f.type == FailureType.SLOW_RESPONSE
    ]
    if slow_failures:
    ...
    if interruption_failures:
        ...
        if high_severity:
            ...
            recommendations.append(
                "Some interruptions detected. Fine-tune voice activity "
                "detection thresholds."
            )

    # Check for ASR confidence issues
    asr_failures = [
        f for f in data.failures.failures if f.type == FailureType.ASR_LOW_CONFIDENCE
    ]
    if asr_failures:
        recommendations.append(
...
def generate_markdown_report(data: ReportData) -> str:
    ...
    if data.analysis.eval_metrics.total_evals > 0:
        ...
        if data.analysis.eval_metrics.intent_correct_rate is not None:
            lines.append(
                f"- Intent correct: {data.analysis.eval_metrics.intent_correct_rate:.1f}%"
            )
        if data.analysis.eval_metrics.avg_relevance_score is not None:
            lines.append(
                f"- Relevance (avg): {data.analysis.eval_metrics.avg_relevance_score:.2f}"
            )
...
def generate_html_report(data: ReportData) -> str:
    ...
    for name, metrics in [
        ...
        if metrics.count > 0:
            ...
            html_parts.append(f"<tr><td>{name}</td><td>0</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>")
    html_parts.append("</table>")
    ...
    html_parts.append('Generated by <a href="https://github.com/voice-observation/voiceobs">voiceobs</a>')
    html_parts.append("</div>")
