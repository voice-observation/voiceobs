...
export default function OrgTestSuiteDetailPage() {
  const router = useRouter();
  const params = useParams();
  const orgId = params.orgId as string;
  const suiteId = params.id as string;
  const suitesBasePath = `/orgs/${orgId}/test-suites`;
  ...
  const fetchData = useCallback(
    async (showLoading = true) => {
      try {
        ...
        setError(null);
        if (!orgId) return;
        const suiteData = await api.testSuites.getTestSuite(orgId, suiteId);
        setSuite(suiteData);
      } catch (err) {
  ...
  const handleGenerateMore = async (prompt?: string) => {
    ...
    try {
      await api.testSuites.generateMoreScenarios(orgId, suite.id, prompt ? { prompt } : undefined);
  ...
  if (error || !suite) {
    return (
      <div className="space-y-6 p-8">
        <Button variant="ghost" size="icon" onClick={() => router.push(suitesBasePath)}>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <Button variant="outline" className="mt-4" onClick={() => router.push(suitesBasePath)}>
              Back to Test Suites
            </Button>
          </CardContent>
        </Card>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          <Button
            ...
            size="icon"
            className="mt-1"
            onClick={() => router.push(suitesBasePath)}
          >
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
